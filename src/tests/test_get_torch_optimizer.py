...
_PARAMETERS: Iterable[torch.Tensor] = [torch.rand(size=_TEST_TUPLE_SIZE), ]

# TODO: a comprehensive list of optimizers?
...
_VALID_OPTIMIZERS: List[str] = _EXACT_OPTIMIZERS + _FUZZY_OPTIMIZERS
...
class TestGetTorchOptimizer(unittest.TestCase):
    def test_get_torch_optimizer(self):
        ...
        for _optimizer in _VALID_OPTIMIZERS:
            assert isinstance(
                get_torch_optimizer(
